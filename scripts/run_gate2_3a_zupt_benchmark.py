"""
NAVRIS Phase 2.3B Gate 2.3A: Real-Data ZUPT A/B Benchmark.

Compares:
- Configuration A: Frozen Gate 2.2 calibrated ESKF (Control, No ZUPT)
- Configuration B: Identical calibrated ESKF + Causal ZUPT (Experiment)
...
2. Identical calibration, initialization, synchronization, and evaluation windows.
3. Pre-declared frozen ZUPT detector thresholds and measurement noise R_zupt.
4. Zero VBOX / future data in runtime detector or filter.
5. Configuration A must reproduce Gate 2.2 baseline bit-for-bit.
"""
...
# Common Predeclared ESKF Configuration (Identical to Gate 2.1 and Gate 2.2)
...
def calibrate_recording_causal(
    ...
    """Causal calibration identical to Gate 2.2."""
...
def run_pipeline_configuration(
    ...
    """
    Executes the real-data ESKF pipeline either as Control (enable_zupt=False)
    or Experiment (enable_zupt=True).
    
    Returns:
        (metrics_dict, gnss_update_logs, zupt_update_logs, zupt_event_logs)
    """
    if rec_id == 'S1':
    ...
    # Initialize Causal Stationary Detector if enabled
    ...
    in_stationary_event = False
    current_event_start = None
    current_event_updates = 0
    current_event_accepted = 0

    acc_count = 0
    rej_count = 0
    nises = []

    # Consecutive GNSS rejection tracking
    ...
    for i in range(n_samples):
        ...
        # 2. Causal ZUPT Update (Configuration B only)
        if enable_zupt and detector is not None:
            is_stat = detector.update(raw_fb, raw_wb)
            
            if is_stat:
                if not in_stationary_event:
                    in_stationary_event = True
                    current_event_start = t
                    current_event_updates = 0
                    current_event_accepted = 0

                v_prior = eskf.state.v.copy()
                p_prior = eskf.state.p.copy()
                q_prior = eskf.state.q.copy()
                ba_prior = eskf.state.ba.copy()
                bg_prior = eskf.state.bg.copy()

                # Apply ZUPT update
                z_acc, z_record = apply_zupt_update(eskf, timestamp=t)
                current_event_updates += 1
                if z_acc:
                    current_event_accepted += 1

                v_post = eskf.state.v.copy()
                ba_post = eskf.state.ba.copy()
                bg_post = eskf.state.bg.copy()
                ...
                zupt_update_logs.append({
                    ...
                    'ba_norm_prior_mps2': float(np.linalg.norm(ba_prior)),
                    'ba_norm_post_mps2': float(np.linalg.norm(ba_post)),
                    'bg_norm_prior_radps': float(np.linalg.norm(bg_prior)),
                    'bg_norm_post_radps': float(np.linalg.norm(bg_post)),
                })
            else:
                if in_stationary_event:
                    # Close stationary event
                    in_stationary_event = False
                    ev_dur = t - current_event_start
                    zupt_event_logs.append({
                        'event_start_s': current_event_start,
                        'event_end_s': t,
                        'duration_s': ev_dur,
                        'total_updates': current_event_updates,
                        'accepted_updates': current_event_accepted,
                        'acceptance_pct': (current_event_accepted / max(1, current_event_updates)) * 100.0
                    })
    ...
    if in_stationary_event:
        ev_dur = t_eval_end - current_event_start
        zupt_event_logs.append({
            'event_start_s': current_event_start,
            'event_end_s': t_eval_end,
            'duration_s': ev_dur,
            'total_updates': current_event_updates,
            'accepted_updates': current_event_accepted,
            'acceptance_pct': (current_event_accepted / max(1, current_event_updates)) * 100.0
    ...
    metrics = {
        'status': 'SUCCESS',
        'recording_id': rec_id,
        'configuration': 'B_EXPERIMENT_ZUPT' if enable_zupt else 'A_CONTROL_BASELINE',
    ...
    if enable_zupt:
        ...
        metrics.update({
            'zupt_detected_events_count': len(zupt_event_logs),
            'zupt_total_duration_s': sum(e['duration_s'] for e in zupt_event_logs),
            'zupt_updates_attempted': z_attempts,
            'zupt_updates_accepted': z_accs,
            'zupt_updates_rejected': z_rejs,
            'zupt_acceptance_pct': (z_accs / max(1, z_attempts)) * 100.0,
            'zupt_median_nis': float(np.median(z_nises)) if z_nises else np.nan,
            'zupt_max_nis': float(np.max(z_nises)) if z_nises else np.nan,
        })

    return metrics, gnss_update_logs, zupt_update_logs, zupt_event_logs


def main():
    print("================================================================================")
    print("NAVRIS Phase 2.3B Gate 2.3A: Real-Data Causal ZUPT A/B Benchmark")
    print("================================================================================")
    ...
    for rec in recordings:
        ...
        if not is_obs:
            print(f"{rec}: UNOBSERVABLE (Class F). Skipping execution.")
            for cfg_name in ['A_CONTROL_BASELINE', 'B_EXPERIMENT_ZUPT']:
                summary_rows.append({
                    'recording_id': rec,
        ...
        # -------------------------------------------------------------
        # 1. RUN CONFIGURATION A (CONTROL: No ZUPT)
        # -------------------------------------------------------------
        print(f"Running Configuration A (Control: Frozen Gate 2.2 Baseline)...")
        m_A, gnss_A, _, _ = run_pipeline_configuration(
            rec, df, calib_res, t_eval_start, enable_zupt=False
        )
        summary_rows.append(m_A)
        print(f"  Config A -> H-RMSE: {m_A['horiz_rmse_m']:.2f} m | Final: {m_A['final_horiz_m']:.2f} m | GNSS Acc: {m_A['gnss_accepted_fixes']}/{m_A['gnss_total_novel_fixes']} ({m_A['gnss_acceptance_pct']:.1f}%)")

        # -------------------------------------------------------------
        # 2. RUN CONFIGURATION B (EXPERIMENT: Causal ZUPT)
        # -------------------------------------------------------------
        print(f"Running Configuration B (Experiment: Causal ZUPT Enabled)...")
        m_B, gnss_B, zupt_updates_B, zupt_events_B = run_pipeline_configuration(
            rec, df, calib_res, t_eval_start, enable_zupt=True
        )
        summary_rows.append(m_B)
        print(f"  Config B -> H-RMSE: {m_B['horiz_rmse_m']:.2f} m | Final: {m_B['final_horiz_m']:.2f} m | GNSS Acc: {m_B['gnss_accepted_fixes']}/{m_B['gnss_total_novel_fixes']} ({m_B['gnss_acceptance_pct']:.1f}%)")
        print(f"  ZUPT: {m_B['zupt_detected_events_count']} events ({m_B['zupt_total_duration_s']:.1f}s) | Updates: {m_B['zupt_updates_accepted']}/{m_B['zupt_updates_attempted']} accepted ({m_B['zupt_acceptance_pct']:.1f}%)")

        # Attach recording ID to logs
        for u in zupt_updates_B:
            u['recording_id'] = rec
            all_zupt_updates.append(u)
        for e in zupt_events_B:
            e['recording_id'] = rec
            all_zupt_events.append(e)
    ...
    summary_csv = os.path.join(out_dir, "gate2_3a_summary.csv")
    summary_json = os.path.join(out_dir, "gate2_3a_summary.json")
    summary_df.to_csv(summary_csv, index=False)
    summary_df.to_json(summary_json, orient="records", indent=2)

    # Save ZUPT logs
    if all_zupt_updates:
        pd.DataFrame(all_zupt_updates).to_csv(os.path.join(out_dir, "zupt_updates.csv"), index=False)
    if all_zupt_events:
        pd.DataFrame(all_zupt_events).to_csv(os.path.join(out_dir, "zupt_events.csv"), index=False)

    print("\n================================================================================")
    print("BENCHMARK EXECUTION COMPLETE. Summary saved to:")
    print(f"  {summary_csv}")
    print("================================================================================")
